# Dice Chess Bot — Cloudflare Greedy — AI Agent Guidelines

## Architecture Overview
- **Domain**: Production Dice Chess ladder anchor running the engine's unmodified `greedy` search on Cloudflare Workers.
- **Contract**: Pure webhook endpoint implementing WebCrypto HMAC-SHA256 verification and ownership handshake.
- **Core Rule**: Do not modify or optimize the strategy; `greedy` is deliberately fixed to act as an unmoving baseline.

## Developer Workflows
- **Setup**: `npm install` (Node.js 24+, Node 26 recommended).
- **Tests**: `npm test` (unit tests against WebCrypto HMAC verification, handshake, and greedy strategy).
- **Typecheck**: `npm run typecheck` (`tsc --noEmit`).
- **Dev**: `npm run dev` (local execution via `wrangler dev` in workerd).
- **Deploy**: `npm run deploy` (Cloudflare Workers deploy).
